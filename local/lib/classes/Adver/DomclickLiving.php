...
require_once $_SERVER['DOCUMENT_ROOT'] . '/poligon/classes/Dadata.php';


class DomclickLiving
{
    ...
    private static function rsInfo($rs)
    {
        ...
        if (($rs['UF_CS_RS_PREMISES_TYPE']['ID'] == 3 || $rs['UF_CS_RS_PREMISES_TYPE']['ID'] == 6) && $plot['UF_CS_PS_PLOT_AREA']) {
            $buildInfo['lot-area'] = [
    ...
    private static function generalInfo(array $rs)
    {
        ...
        if (!empty($plot['UF_CS_PL_CADASTRAL_NUMBER'])) {
            $result['lot-cadastral-number'] = $plot['UF_CS_PL_CADASTRAL_NUMBER'];
        }
        ...
        if (!empty($premises['UF_CS_P_NUMBER'])) {
            $result['apartment'] = $premises['UF_CS_P_NUMBER'];
        }
