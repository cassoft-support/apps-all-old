...
class smartProcessAuth
{
    ...
    public function smartFieldsAdd($params){
        ...
        foreach ($params as $kSmart => $vSmart){
            ...
            foreach ($vSmart as $keyP => $valP){
                ...
                $userfieldList = $this->auth->core->call("userfieldconfig.list",$paramsList)->getResponseData()->getResult()->getResultData();
    ...
    public function securitySmartSmartIndividual($entity, $security, $individual, $categoryId, $limit, $driversAll, $carCheckDate ){
        $guide = $this->smartGuide(); //справочник смартпроцессов
        $entityId = $guide['Проверка контрагентов']['id'];
        file_put_contents($this->fileLog, print_r($limit, true));

        file_put_contents($this->fileLog, print_r("\nsmart", true), FILE_APPEND);
        file_put_contents($this->fileLog, print_r($this->smart  , true), FILE_APPEND);

        file_put_contents($this->fileLog, print_r("\nindividual", true), FILE_APPEND);
        file_put_contents($this->fileLog, print_r($individual, true), FILE_APPEND);
        ...
        file_put_contents($this->fileLog, print_r('parentSmart - ', true), FILE_APPEND);
        file_put_contents($this->fileLog, print_r($parentSmart, true), FILE_APPEND);
        file_put_contents($this->fileLog, print_r("\nparamSmart", true), FILE_APPEND);
        file_put_contents($this->fileLog, print_r($paramSmart, true), FILE_APPEND);
        $smartId = $this->auth->core->call("crm.item.add", $paramSmart)->getResponseData()->getResult()->getResultData()['item']['id'];
        if($smartId){
            $params = [
            ...
            $smartUp = $this->auth->core->call("crm.item.update", ['entityTypeId'=> $this->smart['entityTypeId'],'id' => $this->smart['id'], 'fields' => $params])->getResponseData()->getResult()->getResultData();
            file_put_contents($this->fileLog, print_r("\nindividualUp\n", true), FILE_APPEND);
            file_put_contents($this->fileLog, print_r($individualUp, true), FILE_APPEND);

            file_put_contents($this->fileLog, print_r("\nsmartUp\n", true), FILE_APPEND);
            file_put_contents($this->fileLog, print_r($smartUp, true), FILE_APPEND);

            file_put_contents($this->fileLog, print_r("\nentityTypeId\n", true), FILE_APPEND);
            file_put_contents($this->fileLog, print_r($this->smart['entityTypeId'], true), FILE_APPEND);
...
//            file_put_contents($this->fileLog, print_r($commentAdd, true), FILE_APPEND);
            ...
            file_put_contents($this->fileLog, print_r("testItem\n", true), FILE_APPEND);
            file_put_contents($this->fileLog, print_r($testItem, true), FILE_APPEND);
        }
...
//генерация документа
    function smartDocGenerator( $values, $template, $stamps )
    {
        ...
        $resTemplateAdd = $this->auth->core->call('crm.documentgenerator.document.add', ['templateId' => $guide[$template], 'entityTypeId' => $smart['entityTypeId'], 'entityId' => $smart['id'], 'values' => $values, 'stampsEnabled' => $stamps])->getResponseData()->getResult()->getResultData()['document']['id'];
    ...
    {
        $smart = $this->smart;
        $deal = $this->auth->core->call("crm.deal.get", array('id' => $smart['parentId2']))->getResponseData()->getResult()->getResultData();
        $stageID = explode(':', $deal['STAGE_ID'])[0] . ":".$stage;
        $params = [
            'STAGE_ID' => $stageID,
        ];
        $dealUp = $this->auth->core->call("crm.deal.update", ['id' => $smart['parentId2'], 'fields' => $params]);
        return $dealUp;
