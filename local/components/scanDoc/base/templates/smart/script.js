...
function resizeFrame() {
    ...
    var FrameWidth = document.getElementById("scanFormBlock").offsetWidth;
    console.log(document.getElementById("scanFormBlock"))
...
$('#save').on('click', function() {
  ...
  //  var authParams = Array();
    ...
    formData.append("auth", JSON.stringify(authParams[0]));
    formData.append('app', $('#app').val());
    formData.append('smartElId', $('#smartElId').val());
    formData.append('smartId', $('#smartId').val());
    formData.append('entityTypeId', $('#entityTypeId').val());
      console.log(formData)
    $.ajax({
        url: "/local/components/scanDoc/base/templates/smart/save.php",
        type: "POST",
      /*  data: {
            data:formData,
