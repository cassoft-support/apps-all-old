<?php
    ...
    //require_once($_SERVER["DOCUMENT_ROOT"] . "/local/lib/classes/gf.php");
      require_once($_SERVER["DOCUMENT_ROOT"] . "/local/components/scanDoc/base/templates/deal/save_function.php");
...
if(!empty($clientApp) && $_POST['app']) {
    ...
    $dealUp = $auth->core->call("crm.deal.update", [ 'id' => $_POST['deal_id'], 'fields' => $paramsUp])->getResponseData()->getResult()->getResultData();
    file_put_contents($file_log, print_r($dealUp, true), FILE_APPEND);

echo json_encode($dealUp);
    }
?>
