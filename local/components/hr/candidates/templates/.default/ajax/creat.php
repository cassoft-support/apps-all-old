...
//    d($id);
    $arDealID = json_decode($id, true)['DealId'];
    $arAppID = json_decode($id, true)['AppId'];
    $arSmartID = json_decode($id, true)['SmartId'];
    $companyID = json_decode($id, true)['company'];
    $contactID = json_decode($id, true)['id'];
    $providerType = json_decode($id, true)['providerType'];
    $AppType = json_decode($id, true)['AppType'];
    $TypeIdTest = json_decode($id, true)['TypeId'];
}
//{"id":106,"providerType":"company","contactType":"Provider","company":"72","AppType":"driver"}
if($contactID){
   $header = "РЕДАКТИРОВАНИЕ КАНДИДАТА";
}

if($arAppID !== "driver" && !$contactID){
    $header = "СОЗДАНИЕ КАНДИДАТА";
}
...
<div id="slider-card" class="slider-card " >
    ...
    <div class="block-flex-columns-center-center">
       ...
       <input id="reg" value='<?= $arResult['req'] ?>' style="display:none;" name="reg">
        ...
        <input id="memberId" value='<?= $_POST['member_id'] ?>' style="display:none;" name="memberId">
        <input  type="text" style="display: none;" value="<?= $typeId ?>" id="TYPE_ID" name="TYPE_ID">
        <input  type="text" style="display: none;" value="<?=$companyID?>" id="COMPANY_ID" name="COMPANY_ID">
        <input  type="text" style="display: none;" value="<?=$companyID?>" id="provider" name="provider">
        <input  type="text" style="display: none;" value="<?=$providerType?>" id="providerType" name="providerType">
        <input  type="text" style="display: none;" value="<?=$AppType?>" id="AppType" name="AppType">
...
<script defer src="/local/lib/classes/validate/validateReq.js"></script>
    <script defer type="text/javascript" src="/local/components/hr/candidates/templates/.default/ajax/creat.js?1207"></script>
