...
function savePhoto($auth, $member, $post, $files, $type)
{
 ...
 //  file_put_contents("logImg.txt", print_r($post, true), FILE_APPEND);
   ...
   // $arImage = $files['file'];
    ...
    $folderApp = $auth->core->call('disk.storage.getforapp', []);
    ...
    //    $tmp = $files['file'];
        ...
        foreach ($files['file']['name'] as $key_files => $item_n_files) { // Перебор входящих файлов
           ...
           $Resize->resizePhoto($tempFile, $nameFile);
            ...
            $uploadImage = $auth->core->call('disk.folder.uploadfile', $additionalParameters);
            $uploadImageResult = $uploadImage->getResponseData()->getResult()->getResultData();
         //   $test[] =  $uploadImageResult;
            ...
            if ($uploadImageResult["CONTENT_URL"]) {
                ...
                $ExtLink = $auth->core->call('disk.file.getExternalLink', $additionalParameters);
                $ExtLinkResult = $ExtLink->getResponseData()->getResult()->getResultData();
