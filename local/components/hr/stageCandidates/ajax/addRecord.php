...
$file_log = "/home/bitrix/www/local/components/brokci_settings/favouritesStage/ajax/logAdd.txt";

file_put_contents($file_log, print_r("add",true));
file_put_contents($file_log, print_r($_POST,true), FILE_APPEND);   
$UserA=json_decode($_POST['UserAutPlan'], true);
$UserAll=json_decode($_POST['userAll'], true);
file_put_contents($file_log, print_r($UserA,true), FILE_APPEND);   
 $CRest = new CSRest();
...
$code=translit($_POST['NAME']);
 ...
 $entityAdd = CSRest::call($method, $params);
 file_put_contents($file_log, print_r($entityAdd,true), FILE_APPEND); 
 $arParam['UserAut']=$UserA;
 if($entityAdd['result']>0)
 {
