...
$resElement = $auth->CScore->call( 'crm.item.get', [ 'entityTypeId' => 2, 'id' => $id ])['item'];
...
//p($user , "user", $log);
$name = "[B]сделки[/B] ID".$resElement['ID'].": [URL=/crm/deal/details/".$resElement['id']."/]".$resElement['title']."[/URL], пользователем ".$user['NAME']." ".$user['LAST_NAME'];
p($name , "name", $log);
