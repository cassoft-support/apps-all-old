...
$setup = json_decode($resSetup['CS_QUOTE'], true);
p($setup, "resCompanySetup", $log);
//$resElement = $auth->CScore->call("crm.contact.get", ['ID' => $id, ]);
$resElement = $auth->CScore->call( 'crm.item.get', [ 'entityTypeId' => 7, 'id' => $id ])['item'];
...
//p($user , "user", $log);
$name = "[URL=/crm/type/7/details/".$resElement['ID']."/]Предложения ID".$resElement['id'].", ".$resElement['title']."[/URL], пользователем ".$user['NAME']." ".$user['LAST_NAME'];
p($name , "name", $log);
...
//p($t, "t", $log);
if (!empty($setup['company']) && $resElement['companyId']>0) {
...
p($resElement['contactId'], "contactEl", $log);
if (!empty($setup['contact']) && $resElement['contactId']>0) {
    p($resElement['contactId'], "contactEl", $log);
...
if (!empty($setup['deal']) && $resElement['dealId']>0) {
      $filterDeal['id'] = $resElement['dealId'];
      $filterDeal["closed"] = 'N';
...
//p($t, "t", $log);
if (!empty($setup['lead']) && $resElement['leadId']>0) {
    $filterLead['id'] = $resElement['leadId'];
...
//    $quote = $changeAssigned->changeAssigned($filterQuote, 'quote', 7, $user['ID'], $name);
//    p($quote, "quote", $log);
//}
