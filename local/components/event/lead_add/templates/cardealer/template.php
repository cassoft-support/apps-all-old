...
if ($arParams['app'] && $memberId) {
   ...
   // if (($lead['SOURCE_ID'] == 5 || $lead['SOURCE_ID'] == 3) && $lead['STATUS_SEMANTIC_ID'] === 'P' && $lead['ASSIGNED_BY_ID'] == 27) {
    if ($lead['STATUS_SEMANTIC_ID'] === 'P' && $lead['ASSIGNED_BY_ID'] == 234150) {
        $filter=[
            "ACTIVE" => 'Y',
...
//            }
            if($value["PROPERTY_VALUES"]["rating"]>0) {
                ...
                if ($value["PROPERTY_VALUES"]["volume"] > 0) {
                    $val = $value["PROPERTY_VALUES"]["volume"] * 100 / $value["PROPERTY_VALUES"]["rating"];
