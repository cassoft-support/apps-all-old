...
class TextManager {
  ...
  toDateTime(dateString, template, opts) {
    if (!(typeof template === "undefined") && Type.isStringFilled(template)) {
      return luxon.DateTime.fromFormat(dateString, template, opts);
    }
    return luxon.DateTime.fromISO(dateString, opts);
  }
  getDateForLog() {
        ...
        if (problemError.status === 401 && ["expired_token", "invalid_token"].includes(
          ...
          return this.#clientAxios.post(
            ...
            async (error__) => {
              ...
              if (error__ instanceof axios.AxiosError && error__.response && error__.response.data) {
