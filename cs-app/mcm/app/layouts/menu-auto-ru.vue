...
const items = [
  {
    ...
    to: '/auto-ru/statistics',
    disabled: true,
  },
  {
    label: 'Настройки профилей',
    icon: SettingsIcon,
    to: '/auto-ru/profile_setting'
  },
  {
    label: 'Отчеты (скоро)',
    icon: MailSentIcon,
...
const helpItems = computed(() => {
  ...
  if (currentUser.value?.LAST_NAME === 'Черкасов' ||currentUser.value?.LAST_NAME === 'CAS-soft' ||currentUser.value?.LAST_NAME === 'Support') {
    result.push({
...
<template>
  ...
  >
    <template #sidebar="{ handleClick }" style="margin-right: 10px">
      <B24SidebarHeader>
        <B24SidebarSection class="ps-[18px] flex-row  items-center justify-start gap-0.5 text-red-600">
