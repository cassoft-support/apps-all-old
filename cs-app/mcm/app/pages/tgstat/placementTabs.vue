...
if (authData) {
  const resAuthCheck = await authCheck(authData.member_id, 'tgstat')
    // const data = {
    ...
    //     result.then((value) => {
            if (resAuthCheck === "Y") {
              ...
              if($b24.placement.title === "CRM_DEAL_DETAIL_TAB"
                   || $b24.placement.title ==="CRM_LEAD_DETAIL_TAB"
                   || $b24.placement.title ==="CRM_CONTACT_DETAIL_TAB"
                   || $b24.placement.title ==="CRM_COMPANY_DETAIL_TAB"
                   || $b24.placement.title ==="IM_NAVIGATION"
              ){
                router.push('/tgstat/eventsTab');
              }else{
                router.push('/tgstat/events');
              }
