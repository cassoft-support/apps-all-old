...
if (authData) {
 const resAuthCheck = await authCheck(authData.member_id, "mcm", 'tgstat')
  console.log(resAuthCheck,'resAuthCheck')
            if (resAuthCheck === "Y") {
                console.log(resAuthCheck, 'tgIndex');
              //  router.push('/tgstat/statistics'); // Используем router для навигации
                router.push('/tgstat/events'); // Используем router для навигации
            } else {
