...
// ✅ Создаём объект: ID -> LINE_NAME
    ...
    profileAll.value = await Promise.all(
        setup.map(async item => {
          ...
          let phoneProfile = ''
          if (profileId && !values.CS_DATE_CLOSE_FACT) {
            const profileData = await profileStatus(profileId, type)
            if (profileData && profileData.phone) {
               phoneProfile = profileData.phone
            }
            if (profileData && profileData.authorized) {
              authorized = true

            }
            console.log(profileData.status, 'profileData!!')
...
// Показываем подтверждение
  const confirmed = confirm(`Вы действительно хотите деактивировать профиль "${profile.name}"?`);

  if (!confirmed) {
    return; // Отмена
  }

  console.log('Удаляем профиль:', profile)

// Определяем URL в зависимости от типа профиля
  let profileTypeUrl = ''
  if (profile.type === 'Whatsapp' || profile.type === 'whatsapp') {
    profileTypeUrl = '/api'
  } else {
    profileTypeUrl = '/tapi'
  }
...
<div class="flex flex-row">
              <div  class="  rounded w-6 h-6 shadow-md rounded mr-4">
  ...
  <div  class="  rounded w-6 h-6 shadow-md rounded">
                <B24Icon name="Main::TrashBinIcon" class="w-6 h-6 color-gray-400"
                         @click.stop="async () => { return deleteProfile(profile) }"
                />
              </div>
</div>
            </div>
