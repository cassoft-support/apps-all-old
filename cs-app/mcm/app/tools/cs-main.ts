...
//добавляем к дате 30 дней
export  function addDate30(val: number): number {
// Создаём новую дату из Unix timestamp
    let date = new Date(val * 1000); // val — в секундах, умножаем на 1000 для JS
    date.setDate(date.getDate() + 30); // Добавляем 30 дней

// Если дата меньше текущей — добавляем ещё 30 дней
    if (date < new Date()) {
