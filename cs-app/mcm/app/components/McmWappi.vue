...
const canSend = computed(() => {
  return newMessage.value.trim() !== '' || file.value !== null;
});
const raw = ref({
  filter: {
    ids: []
...
async function searchDialogs() {
  ...
  try {
    const url = `/api/sync/chats/filter?profile_id=${profileId}&${queryParams.toString()}`;
    const response = await csWappi.wappGet(url);
...
function selectActivity(activity) {
  if (selectedActivityId.value !== activity.id) {
    // Если выбран новый чат, сбросьте текущие сообщения
    messagesResults.value = { messages: [], total_count: 0 };
    loadedMessagesCount.value = 0;
  }

  selectedActivityId.value = activity.id;
  console.log(selectedActivityId.value, 'selectedActivityId');
  messagesUploadList(activity.id, activity.contact.PushName, activity.thumbnail, profileId);
}
...
async function messagesUploadList(chatId, Name, icon, profileId, offset = 0) {
  ...
  try {
    ...
    if (offset > 0 && scrollContainer) {
      // Сохраните текущую позицию прокрутки, если загружаются более старые сообщения
    ...
    const response = await csWappi.wappGet(params);

    // Обновите общее количество сообщений


    // Добавьте новые сообщения в начало списка
    const newMessages = response.messages.reverse();
    messagesResults.value.messages = [...newMessages, ...messagesResults.value.messages];

    // Обновите количество загруженных сообщений
    ...
    nextTick(() => {
      if (scrollContainer) {
        if (offset === 0) {
          ...
          // Восстановите позицию прокрутки для более старых сообщений
...
function addEmoji(emoji) {
  console.log('Selected emoji object:', emoji);
  if (emoji.i) {
    newMessage.value += emoji.i;
  } else {
    console.error('Emoji object does not contain expected properties.');
  }
}
...
var profileId = "c40b7054-9f50";

function sendMessage() {
  if (newMessage.value.trim()) {
    const profileInput = document.querySelector('input[ref="profileInput"]');
    console.log(profileInput, 'profileInput.value');
    const recipientInput = document.querySelector('input[type="file"]');
    const recipient = recipientInput ? recipientInput.dataset.id : '';
    ...
    const raw = JSON.stringify({
      ...
      recipient: messagesResults.value.id
    });
    ...
    const response = fetch(`https://wappi.pro/api/async/message/send?profile_id=${profileId}`, requestOptions);
    console.log(response);
    console.log('Message sent:', newMessage.value);
    newMessage.value = '';
    messagesUploadList(messagesResults.value.id, messagesResults.value.name, messagesResults.value.icon, profileId);
  }
}
    ...
    <div class="mainWindow">
      ...
      <div class="vac-card-window" tasks="" messages-loaded="true"
        ...
        <div class="vac-chat-container">
            ...
            <div data-scrollbar="true" tabindex="-1" style="overflow: hidden; outline: none;">
              <div class="scroll-content flex flex-col">
                <div>
                  ...
                  <div class="showChats">Профиль++: <a href="#" class="selectChats v-popper--has-tooltip">cs_613</a>
                  </div>
                </div>
                          ...
                          <div class="vac-name-container vac-text-ellipsis">
                            ...
                            <div class="vac-text-last">
                              ...
                              <div class="vac-format-message-wrapper vac-text-ellipsis">
                                <div class="vac-format-container vac-text-ellipsis"><span class="vac-text-ellipsis"> но ваш тот портал битрикс удалили вы на нем не были более 3 месяцев, так что все с чистого листа, но может и лучше </span>
                                </div>
                              </div><!---->
          ...
          <div v-if="messagesResults" class="vac-col-messages" messages-loaded="true" style="position: relative;">
            ...
            <div class="vac-room-header vac-app-border-b">
              <div class="vac-room-wrapper">
                <div class="vac-info-wrapper">
                  <div class="vac-avatar" :style="{ backgroundImage: `url(${messagesResults.icon})` }"></div>
                  <div class="vac-text-ellipsis">
                    <div class="vac-room-name vac-text-ellipsis">{{ messagesResults.name }}</div>
                    <div class="vac-room-info vac-text-ellipsis" style="display: flex; align-items: center;">
                      <svg width="16" height="16" viewBox="0 0 22 22" fill="currentColor" xmlns="http://www.w3.org/2000/svg" style="color: rgb(35, 187, 134);">
                        ...
                        <span style="margin-left: 4px; color: rgb(108, 117, 125);">{{ messagesResults.profileId }}</span>
                    </div>
                  </div>
                </div>
            ...
            <div id="room-footer" class="vac-room-footer" style="">
              <div class="vac-box-footer vac-box-footer-border">
                <div class="vac-icon-textarea-left" style="display: none">
                ...
                <div class="vac-icon-textarea">
                  ...
                  <div class="vac-svg-button " @click="triggerFileInput" style="display: none">
                  ...
                  <input type="file" @change="handleFileUpload" accept="*" capture="" style="display: none;">
                  <div :class="['vac-svg-button', { 'vac-send-disabled': !canSend }]" @click="sendMessage" style="margin-top: 2px;">
          ...
          <div class="vac-rooms-container contactWindow vac-app-border-r "
               style="border-top-left-radius: 0px; border-bottom-left-radius: 0px; display: none;">
            <div class="vac-room-header vac-app-border-b"
                 style="position: relative; border-top-right-radius: 0px; border-bottom: 1px rgb(255, 255, 255);">
              <div class="vac-room-wrapper" style="justify-content: end;">
                <div class="contactWindowClose">
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"
                       style="cursor: pointer;">
                    <path d="M1.3335 1.33325L14.6668 14.6666M14.6668 1.33325L1.3335 14.6666" stroke="#1876D2"
            ...
            <div class="contactContainer" >
              <div class="vac-avatar"
                   style="margin-right: 0px !important; width: 100px; height: 100px; background-image: url(&quot;/assets/noavatar.svg&quot;);"></div>
              <div style="margin-top: 16px; font-weight: 500;">Дмитрий</div>
              <div style="display: flex; align-items: center; justify-content: center; margin-top: 16px;">+7 (968)
                180-27-67
              </div><!---->
              <div style="display: flex; align-items: center; justify-content: center; margin-top: 16px;">
                <svg width="16" height="16" viewBox="0 0 22 22" fill="currentColor" xmlns="http://www.w3.org/2000/svg"
                  ...
                  <path
                      d="M11.0027 0H10.9972C4.93211 0 0 4.93349 0 11C0 13.4062 0.775498 15.6365 2.09412 17.4473L0.723248 21.5338L4.95136 20.1822C6.69073 21.3344 8.7656 21.9999 11.0027 21.9999C17.0678 21.9999 21.9999 17.0651 21.9999 11C21.9999 4.93486 17.0678 0 11.0027 0ZM17.4033 15.5333C17.138 16.2827 16.0847 16.9042 15.2446 17.0857C14.6698 17.2081 13.9191 17.3057 11.3918 16.258C8.15923 14.9187 6.07748 11.6338 5.91523 11.4207C5.75986 11.2076 4.60899 9.68135 4.60899 8.10285C4.60899 6.52436 5.41061 5.75573 5.73374 5.42574C5.99911 5.15486 6.43773 5.03111 6.85848 5.03111C6.99461 5.03111 7.11698 5.03799 7.22698 5.04349C7.55011 5.05724 7.71235 5.07649 7.92548 5.58661C8.19085 6.22598 8.8371 7.80448 8.9141 7.96673C8.99248 8.12898 9.07085 8.34898 8.96085 8.5621C8.85773 8.7821 8.76698 8.87973 8.60473 9.06673C8.44248 9.25373 8.28848 9.39673 8.12623 9.59748C7.97773 9.7721 7.80998 9.9591 7.99698 10.2822C8.18398 10.5985 8.83023 11.6531 9.78172 12.5001C11.0096 13.5932 12.0051 13.9425 12.3612 14.091C12.6266 14.201 12.9428 14.1748 13.1367 13.9686C13.3828 13.7032 13.6867 13.2632 13.9961 12.8301C14.2161 12.5193 14.4938 12.4808 14.7853 12.5908C15.0823 12.694 16.654 13.4708 16.9771 13.6317C17.3002 13.794 17.5133 13.871 17.5917 14.0071C17.6687 14.1432 17.6687 14.7826 17.4033 15.5333Z"
                      fill="currentColor"></path>
                </svg><!----><!----><!----><span style="margin-left: 4px; color: rgb(108, 117, 125);">cs_613</span>
              </div>
              <!----></div>
            <div class="vac-room-header vac-app-border-b" style="position: relative; border-top-right-radius: 0px;">
              <div class="vac-room-wrapper"></div>
            </div>
          </div>
        </div><!----></div>
    </div>
