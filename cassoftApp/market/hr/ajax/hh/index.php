...
if (isset($_GET['mode'])) {
    if ($_GET['mode'] === 'publication') {
    $_SESSION["mode"] = $_GET['mode'];
    }
}

if (isset($_GET['mode'])) {
    if ($_GET['mode'] === 'dosubscriptions') {
    $_SESSION["mode"] = $_GET['mode'];
    }
}

if (isset($_GET['mode'])) {
    if ($_GET['mode'] === 'getsubscriptions') {
    $_SESSION["mode"] = $_GET['mode'];
    }
}
if (isset($_GET['mode'])) {
    if ($_GET['mode'] === 'delsubscriptions') {
    $_SESSION["mode"] = $_GET['mode'];
    }
}
...
//***************************************************
//******************  Получение откликов по вакансиям

function getResponseVacancies($access_token)
{
   /* $employer = 3964417; 
...
} // Получение откликов по вакансиям конец
 
 // Функция публикации вакансии
function postVacanciesDrafts($access_token)
{
    ...
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($curl, CURLOPT_HTTPHEADER, ['Authorization: Bearer ' . $access_token, 'user-agent: Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36', 'Content-Type: application/json', 'Content-Length: ' . strlen($data_string) ]);
 ...
 if (isset($_SESSION["mode"]))
{
  ...
  echo '<br>';  
    ...
    postVacanciesDrafts($access_token);
}
 ...
 if ($mode == 'response') {
    ...
    $employer = 3964417; 
    getVacancies($access_token, $employer);
    getResponseVacancies($access_token);
    
 }
    
/* NEW_NEGOTIATION_VACANCY (object) or NEW_RESPONSE_OR_INVITATION_VACANCY (object) or VACANCY_ARCHIVATION (object) or VACANCY_PUBLICATION_FOR_VACANCY_MANAGER (object) or VACANCY_PROLONGATION (object) or VACANCY_CHANGE (object) or NEGOTIATION_EMPLOYER_STATE_CHANGE (object)
...
function postSubscriptions ($access_token)
{
$vac3 = '{
"actions": [
{
...
"url": "https://city.brokci.ru/cassoftApp/market/hr/ajax/hh/index.php?mode=publication"
}';
...
curl_setopt($ch, CURLOPT_URL, 'https://api.hh.ru/webhook/subscriptions');
...
if ($mode == 'dosubscriptions') {
postSubscriptions($access_token);
}
...
// Функция удалить подписку на уведомления
function delSubscriptions ($access_token)
{
$headers = array(
        'Authorization: Bearer ' . $access_token,
        'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.71 Safari/537.36'
...
$ch = curl_init('https://api.hh.ru/webhook/subscriptions/202160');
...
if ($mode == 'delsubscriptions') {
delSubscriptions($access_token);
}




// Функция получить  подписки на уведомления
function getSubscriptions ($access_token)
{
$webhookUrl = 'https://api.hh.ru/webhook/subscriptions';
